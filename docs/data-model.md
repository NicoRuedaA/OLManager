...
| `players_file` | `String` | ✅ | Path to players JSON, relative to `data/` (e.g. `"players/lec_players.json"`). |
| `staff_file` | `Option<String>` | ❌ | Path to competition-specific staff file (optional — global `staffs/free_agents.json` used otherwise). |
